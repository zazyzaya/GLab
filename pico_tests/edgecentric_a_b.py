...
''' Combines events by timecode. If they occured within the same `alpha` of time
	then they are combined. Otherwise we make a new node
'''
def aggregate_by_timecode(G, streamer):
	edge = next(streamer)
	global ALPHA
...
''' Loads a list of files into a graph
'''
def load_list_of_graphs(fnames, saveas=None, add_func=aggregate):
	G = nx.DiGraph()
...
'''
def load_all_graphs(add_func=aggregate):
	G = nx.DiGraph()
