...
def aggregate(G, streamer):
	...
	if (edge['id.orig_h'], edge['id.resp_h']) not in G.edges():
		...
		conn_vec = [0] * len(CONN_DICT)
		orig_pkt_vec = [0] * (MAX_PKTS + PKT_OFFSET)
		resp_pkt_vec = [0] * (MAX_PKTS + PKT_OFFSET)
		dur_vec = [0] * (MAX_DUR + DUR_OFFSET)
		...
		ts_vec = [0] * 21 # Approx number of ln(seconds in a day). Any greater, and we dont care
		last_ts = None
